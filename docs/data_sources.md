...
| Europe PMC | ranking signal | <https://www.ebi.ac.uk/europepmc/webservices/rest> | REST v6.9 | 2026-07-12 | Yes | Gene mention count (`hitCount`) is the score; in discovery mode, gene∧disease co-mention count. When count > 0, also surfaces the top few articles (`search`) as citable per-paper evidence (`PMID:<n>`, or a Europe PMC id for preprints) so curation/specialist rationales can cite real papers |
...
| gnomAD | ranking signal | <https://gnomad.broadinstitute.org/api> | gnomad_r4 (GraphQL) | 2026-07-11 | Yes | LOEUF loss-of-function constraint (annotation, lower is better). Opt-in `gnomad_af`: max allele frequency among the gene's predicted-LoF variants (gene-level rarity; drives the "common variant" caveat). Variant-level rsID allele frequency reserved for a future variant mode |
...
| IMPC | ranking signal (opt-in) | <https://www.ebi.ac.uk/mi/impc/solr> | Solr `gene` + `genotype-phenotype` cores | 2026-07-11 | Yes (CC0) | Model-organism knockout phenotype (model-organism axis). Two GETs keyed off the human symbol: (1) `gene` core `human_gene_symbol:<SYM>` → the mouse ortholog's MGI marker accession (mandatory: the genotype-phenotype core has no human-symbol field); (2) `genotype-phenotype` core → significant knockout phenotype associations. Signal = count of distinct significant phenotype terms (MP/MPATH). A hypothesis-free systematic screen, so NOT a study-popularity proxy (obscure genes can score high, famous ones low). A gene with no IMPC ortholog, or none phenotyped/none significant, is a MISS (never a 0 masquerading as evidence). Annotation (nudges up, never gates). Each phenotype is grounded evidence (knockout allele + MP term id). Pin the IMPC data release when reproducing |
