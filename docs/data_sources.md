...
| cBioPortal | ranking signal (planned) | <https://www.cbioportal.org/api> | TBD | TBD | Yes | Cross-study somatic mutation frequency (cancer). Needs a multi-step molecular-profile + sample-list query — deferred |
| CIViC | ranking signal (planned) | <https://civicdb.org/api/graphql> | TBD | TBD | Yes | Clinical evidence items (cancer, CC0). Needs a symbol→CIViC-gene-id lookup then `gene(id:)` — deferred |
| ClinGen | ranking signal (planned) | <https://search.clinicalgenome.org> | TBD | TBD | Yes | Gene–disease validity + dosage sensitivity. Live API path unconfirmed — deferred |
