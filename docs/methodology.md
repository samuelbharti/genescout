...
- The **composite** is a weighted mean over the *evidence* signals — a missing
  ...
  broadly-supported gene beats a narrow-but-famous one — then *present* annotations
  are folded in and only ever nudge a gene up. Weights live in
...
deterministic ([`R/caveats.R`](../R/caveats.R)) and reproducible — no LLM in the
