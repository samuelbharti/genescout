...
# path are exercised under CANDID_APP_ROOT pointed at the app root (matching how the
...
test_that("candid_byok_providers/meta/choices cover the three key providers", {
  provs <- candid_byok_providers()
  ...
  for (p in provs) {
    meta <- candid_provider_meta(p)
    expect_true(nzchar(meta$label))
    expect_match(meta$key_url, "^https://")
    expect_true(nzchar(meta$env))
  }
  ch <- candid_provider_choices()
  expect_length(ch, 3)
  expect_setequal(unname(ch), provs)
  expect_true(all(nzchar(names(ch)))) # human labels
})

test_that("candid_default_byok_provider prefers a BYOK-capable configured provider", {
  expect_equal(
    candid_default_byok_provider(list(provider = "openai")),
    "openai"
  )
  expect_equal(
    candid_default_byok_provider(list(provider = "anthropic")),
    "anthropic"
  )
  # google_vertex is not a BYOK provider -> fall back to the first offered.
  expect_equal(
    candid_default_byok_provider(list(provider = "google_vertex")),
    candid_byok_providers()[1]
  )
  expect_equal(candid_default_byok_provider(list()), candid_byok_providers()[1])
})
...
test_that("load_byok_models exposes tiered roles + a chat model per provider", {
  ...
  for (p in candid_byok_providers()) {
...
test_that("candid_provider_model_suggestions are unique, non-empty, from config", {
  withr::with_envvar(c(CANDID_APP_ROOT = byok_app_root), {
    s <- candid_provider_model_suggestions("anthropic")
...
test_that("candid_byok_credential trims the key and blanks an empty model to NULL", {
  cr <- candid_byok_credential("anthropic", "  sk-abc  ", model = "  ")
  expect_equal(cr$provider, "anthropic")
  expect_equal(cr$api_key, "sk-abc")
  expect_null(cr$model)
  expect_equal(
    candid_byok_credential("openai", "k", model = "gpt-x")$model,
    "gpt-x"
  )
})

test_that("candid_credential_ready reflects a usable key", {
  expect_false(candid_credential_ready(NULL))
  expect_false(candid_credential_ready(candid_byok_credential("openai", "")))
  expect_true(candid_credential_ready(candid_byok_credential("openai", "k")))
})
...
test_that("byok_effective_config leaves the base config untouched without a key", {
  ...
  expect_identical(
    byok_effective_config(base, candid_byok_credential("openai", "")),
    base
  )
})
...
test_that("byok_effective_config folds provider/models/api_key onto the base", {
  withr::with_envvar(c(CANDID_APP_ROOT = byok_app_root), {
    ...
    eff <- byok_effective_config(
      base,
      candid_byok_credential("anthropic", "sk-1")
    )
...
test_that("a single-model override supersedes every role and the chat model", {
  withr::with_envvar(c(CANDID_APP_ROOT = byok_app_root), {
    cr <- candid_byok_credential("openai", "k", model = "gpt-mine")
    eff <- byok_effective_config(list(), cr)
    expect_true(all(unlist(eff$models) == "gpt-mine"))
    expect_equal(candid_chat_model(cr), "gpt-mine")
  })
})

test_that("candid_chat_model falls back to the provider's configured chat model", {
  withr::with_envvar(c(CANDID_APP_ROOT = byok_app_root), {
    expect_equal(
      candid_chat_model(candid_byok_credential("google_gemini", "k")),
...
test_that("candid_llm_available reads a session api_key on the config", {
  skip_if_not_installed("ellmer")
  withr::with_envvar(c(ANTHROPIC_API_KEY = ""), {
    expect_false(candid_llm_available(list(provider = "anthropic")))
    expect_true(candid_llm_available(list(
      provider = "anthropic",
      api_key = "sk-live"
    )))
  })
...
test_that("candid_redact_secret removes the literal key and is a no-op when empty", {
  expect_equal(
    candid_redact_secret("boom sk-123 fail", "sk-123"),
    "boom <redacted-key> fail"
  )
  expect_equal(candid_redact_secret("nothing here", ""), "nothing here")
  expect_equal(candid_redact_secret(c("a", "b"), ""), "a b")
})

test_that("candid_chat_grounding degrades gracefully with no run", {
  expect_match(candid_chat_grounding(NULL), "No review has been run")
  expect_match(
    candid_chat_grounding(list(genes = data.frame())),
    "No review has been run"
  )
})

test_that("candid_chat_grounding summarizes the current run with grades", {
  genes <- data.frame(
    symbol = c("NF1", "TTN"),
